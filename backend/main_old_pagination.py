...
def validate_embedding_config():
    """Validate that EMBEDDING_MODEL and PINECONE_DIMENSION are compatible"""
    expected_dim = EMBEDDING_DIMENSIONS.get(EMBEDDING_MODEL)
    if expected_dim is None:
        logger.warning(f"Unknown embedding model '{EMBEDDING_MODEL}'. Cannot validate dimensions.")
    elif expected_dim != PINECONE_DIMENSION:
        raise ValueError(
            f"Dimension mismatch: {EMBEDDING_MODEL} produces {expected_dim}-dimensional vectors, "
            f"but PINECONE_DIMENSION is set to {PINECONE_DIMENSION}. "
            f"Please update PINECONE_DIMENSION={expected_dim} in your .env file or change EMBEDDING_MODEL."
        )
    else:
        logger.info(f"Embedding configuration validated: {EMBEDDING_MODEL} → {PINECONE_DIMENSION} dimensions")
...
if ALLOWED_ORIGINS_ENV:
    ...
    logger.warning("CORS configured with allow_origins=['*'] and allow_credentials=False (development mode). Set ALLOWED_ORIGINS in production!")
...
async def lifespan(app: FastAPI):
    ...
    try:
        # Validate embedding configuration before initialization
            ...
            if vectors_to_upsert:
                try:
                    await pinecone_with_retry(
                        lambda batch=vectors_to_upsert: index.upsert(vectors=batch),
                        max_retries=3,
                        timeout=15.0
                    )
    ...
    try:
                ...
                try:
                    ...
                    updated_metadata = {
                        **existing_metadata,  # Keep existing fields
                        ...
                        "all_12_steps": detected_cats.get("12_steps", existing_metadata.get("all_12_steps", [])),
                        ...
                        "all_quantum_physics": detected_cats.get("quantum_physics", existing_metadata.get("all_quantum_physics", [])),
        ...
        for i in range(0, len(vectors_to_update), UPSERT_BATCH_SIZE):
            ...
            if vectors_with_embeddings:
                await pinecone_with_retry(
                    lambda vecs=vectors_with_embeddings: index.upsert(vectors=vecs),
                    max_retries=3,
                    timeout=15.0
                )
